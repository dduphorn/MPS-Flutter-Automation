...
//		CommonANDROID clsCommonMobile = new CommonANDROID();
	...
	//********************************************************************************************************************
  	...
  	//********************************************************************************************************************
  	@Test(priority=3001)
  	public void I3001_ValidateInvalidCredentialMessage()
  	{
  		...
  		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Login", "Populate Login", "{T} Email Id|{T} Password",strUserName+"|"+strPassword);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Login",1);
   		try {Thread.sleep(2500);}catch (Exception e) {}
   		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Login", "Dialog Message-Invalid Credentials.", 1, "Value", "Invalid credentials, please contact customer support.");
  		iosdriver.quit();
  		clsMeter.METER_SetMeterEndTime(objDictionary);
	...
	//Register User
  	@Test(priority=3002)
  	public void I3002_RegisterUser_PasswordShouldHaveAtleastOneDigit()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		WebElement keyboard = iosdriver.findElement(By.className("UIAKeyboard"));
		...
		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		...
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Please Ensure that you have at least one lower case letter, one upper case letter and a digit in your password.", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3003)
  	public void I3003_RegisterUser_EmailHasAlreadyBeenTaken()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		String strIOSUdid = objDictionary.get("strIOSUdid");
   		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Register", "Remember me",1);
  		try {Thread.sleep(500);}catch (Exception e) {}
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "Go to Settings",1);
			try {Thread.sleep(500);}catch (Exception e) {}
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "Location",1);
			try {Thread.sleep(500);}catch (Exception e) {}
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "While Using the App",1);
			try {Thread.sleep(500);}catch (Exception e) {}
			iosdriver.navigate().back();
			try {Thread.sleep(500);}catch (Exception e) {}
			try {
			    List<WebElement> allElements = iosdriver.findElements(By.xpath("//*")); 
			    System.out.println("Number of elements found: " + allElements.size());
			    
			    // Create a list to store the names
			    List<String> elementNames = new ArrayList<>();

			    // Iterate through all elements and store their names
			    for (WebElement element : allElements) {
			        String name = element.getAttribute("name");
			        if (name != null && !name.isEmpty()) 
			        {
			        	if(name.equals("Sentry Mobile"))
				        {
			        		element.click();
				        }
			    ...
			    // Display all names in one list
				System.out.println("Element names: " + elementNames);


			    // Display all names in one list
			    System.out.println("Element names: " + elementNames);
			} catch (NoSuchSessionException e) {
			    System.out.println("Session is not valid: " + e.getMessage());
			    // Handle reinitialization or other logic
			} catch (Exception e) {
			    System.out.println("An error occurred: " + e.getMessage());
			}

			
			WebElement navBar = iosdriver.findElement(By.xpath("//XCUIElementTypeNavigationBar[@name=\"Sentry Mobile\"]"));
			if (navBar.isDisplayed() && navBar.isEnabled()) {
			    navBar.click();
			} else {
			    System.out.println("Element is not visible or not enabled.");
			}
			
			// Get the location of the element
			int x = navBar.getLocation().getX() + (navBar.getSize().getWidth() / 2); // Center of the element
			int y = navBar.getLocation().getY() + (navBar.getSize().getHeight() / 2); // Center of the element
...
//			    .perform();
	        ...
	        action.tap(PointOption.point(x, y)).perform(); // Tap at the specified coordinates
	        ...
	        // Locate the element to move to (replace with your actual button locator)
	      	WebElement buttonElement = iosdriver.findElement(By.xpath("//XCUIElementTypeButton[@name=\"com.apple.Magnifier\"]/XCUIElementTypeImage[1]"));
	        
	     
	        // Perform the action
	        actions.moveToElement(buttonElement) // Move to the button element
	               .moveByOffset(27, 27) // Move the pointer by the specified offsets
			...
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "Sentry Mobile",1);
			try {Thread.sleep(1000);}catch (Exception e) {}
			iosdriver.navigate().back();
			try {Thread.sleep(1000);}catch (Exception e) {}
			iosdriver.navigate().back();
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "Sentry Mobile Back To App",1);
			try {Thread.sleep(500);}catch (Exception e) {}
		}
		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "message Unable to create User", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3004)
  	public void I3004_RegisterUser_EmailIsInvalid()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
		...
		try {iosdriver.findElement(By.name("I have left")).click();}catch(Exception a) {}
		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
			clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "Go to Settings",1);
			String strIOSUdid = objDictionary.get("strIOSUdid");
			if(!strIOSUdid.equals(""))
			{
				clsCommonMobile.SENTRYMOBILE_EnableLocationServices(objDictionary,iosdriver);
				try {Thread.sleep(500);}catch (Exception e) {}
				clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
			}
		}
		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "email is invalid", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3005)
  	public void I3005_RegisterUser_EmailIdsDoNotMatch()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		String strPassword = "L@kemaryMN03";
   		...
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|s"+strUserName+"|"+strPassword+"|"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
  		...
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Email ids do not match.", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3006)
  	public void I3006_RegisterUser_PasswordsDoNotMatchPleaseReEnter()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		String strPassword = "L@kemaryMN03";
   		...
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|s"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Register", "Remember me",1);
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
  		...
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Passwords do not match. Please re-enter and submit again", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3007)
  	public void I3007_RegisterUser_PasswordShouldBeAtleast12Characters()
  	{
		...
		if(strCondition.equals("True"))
		{
  		...
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Password should be at least 12 characters", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3008)
  	public void I3008_RegisterUser_FirstNameCannotBeBlank()
  	{
		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		String strPassword = "L@kemaryMN03";
   		...
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
  		...
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "All fields are mandatory.", 1, "Exists", "");
  		iosdriver.quit();
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3009)
  	public void I3009_RegisterUser_LastNameCannotBeBlank()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete||"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		String strCondition = clsCommonMobile.ConditionalStepText(objDictionary, iosdriver, "Register", "Access to location has been denied. In order to provide you the best service possible, Please enable access in Settings.", 1, "Exists", "");
		if(strCondition.equals("True"))
		{
  	...
  	@Test(priority=3010)
  	public void I3010_RegisterUser_RememberMeOff_EmailIdWithCaps_LoginWithCaps()
  	{
		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  		...
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		//Destroy User
  		String strUserName = "IDeleteUser10b@gmail.com";objDictionary.put("strUserName", strUserName);
  		String strPassword = "IDeleteMe10b";
  		clsCommonWeb.SENTRYLINK_DestroyUser2(objDictionary, strUserName, strPassword);
   		//Register User Email With Caps
   		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
		clsCommonMobile.ToggleButton(objDictionary, iosdriver, "Register", "Remember me", false);
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		try {Thread.sleep(1000);}catch (Exception e) {}
   		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.", 1, "Value", "You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.");
  	...
  	@Test(priority=3011)
  	public void I3011_RegisterUser_RememberMeOff_EmailIdWithCaps_LoginWithoutCaps()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  		...
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		//Destroy User
  		String strUserName = "IDeleteUser11b@gmail.com";objDictionary.put("strUserName", strUserName);
  		String strPassword = "IDeleteMe11b";
  		clsCommonWeb.SENTRYLINK_DestroyUser2(objDictionary, strUserName, strPassword);
   		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
		...
		clsCommonMobile.ToggleButton(objDictionary, iosdriver, "Register", "Remember me", false );
  		...
  		clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "OK",1);
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Login", "Populate Login", "{T} Email Id|{T} Password",strUserName.toLowerCase()+"|"+strPassword);
  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Login",1);
  	...
  	@Test(priority=3012)
  	public void I3012_RegisterUser_RememberMeOff_EmailIdWithoutCaps_LoginWithCaps()
  	{
		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  		...
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		//Destroy User
  		String strUserName = "IDeleteUser3@gmail.com";objDictionary.put("strUserName", strUserName);
  		String strPassword = "IDeleteMeMN03";
  		//Check if User is deleted in user - Not Coded
  		clsCommonWeb.SENTRYLINK_DestroyUser2(objDictionary, strUserName, strPassword);
   		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		String strMunicipality = objDictionary.get("strMunicipality");
  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName.toLowerCase()+"|"+strUserName.toLowerCase()+"|"+strPassword+"|"+strPassword+"");
		clsCommonMobile.ToggleButton(objDictionary, iosdriver, "Register", "Remember me", false);
  		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Dialog Message-You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.", 1, "Value", "You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.");
  	...
  	@Test(priority=3013)
  	public void I3013_RegisterUser_RememberMeOff_EmailIdWithoutCaps_LoginWithoutCaps()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  	...
  	@Test(priority=3014)
  	public void  I3014_RegisterUser_RememberMeOn_EmailIdWithCaps()
  	{
		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  		...
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		//Destroy User
  		String strUserName = "IDeleteUser14b@gmail.com";objDictionary.put("strUserName", strUserName);
  		String strPassword = "IDeleteMeMN14b";
  		clsCommonWeb.SENTRYLINK_DestroyUser2(objDictionary, strUserName, strPassword);
   		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		String strMunicipality = objDictionary.get("strMunicipality");
  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Register",1);
  		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Register", "Populate Login", "{T} First Name|{T} Last Name|{T} Email Id|{T} Confirm Email Id|{T} Password|{T} Confirm Password","Delete|User|"+strUserName+"|"+strUserName+"|"+strPassword+"|"+strPassword+"");
		clsCommonMobile.ToggleButton(objDictionary, iosdriver, "Register", "Remember me", true);
		clsCommonMobile.ClickButton(objDictionary,iosdriver, "Register", "Submit", 1);
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "Dialog Message-You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.", 1, "Value", "You have successfully registered! Click \"OK\" to login. After logging in, you can go to \"Account\" to add money to your account.");
  		clsCommonMobile.ClickLink(objDictionary, iosdriver, "Register", "OK",1);
  	...
  	@Test(priority=3015)
  	public void I3015_RegisterUser_RememberMeOn_EmailIdWithoutCaps()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		CommonWeb clsCommonWeb = new CommonWeb();
  	...
  	@Test(priority=3016)
  	public void I3016_CreateAndDeleteUser_AttemptMobileRegisterWithDeletedUser()
  	{
  		objDictionary.put("strAssociatedBug", "169601821");
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonWeb clsCommonWeb = new CommonWeb();
  		CommonIOS clsCommonMobile = new CommonIOS();
  		Meter clsMeter = new Meter();
  		...
  		String strUserName = "IDeletedUser2@gmail.com";
  		String strPassword = "IDeletedMN@01";
		...
		if(strCondition.equals("True"))
		{
  		...
  		try {Thread.sleep(1000);}catch (Exception e) {}
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Register", "email has already been taken", 1, "Exists", "");
  		iosdriver.quit();
   		clsMeter.METER_SetMeterEndTime(objDictionary);
   		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	...
  	@Test(priority=3017)
  	public void I3017_CreateAndDeleteUser_AttemptLoginWithDeletedUser()
  	{
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonWeb clsCommonWeb = new CommonWeb();
  		CommonIOS clsCommonMobile = new CommonIOS();
  		...
  		WebDriver driver = getDriver();
  		clsCommonWeb.SENTRYLINK_OpenLoginPage(objDictionary, driver);
  		String strUserName = "IDeletedUser017@gmail.com";
  		String strPassword = "IDeletedMN@17!";
  		clsCommonWeb.SENTRYLINK_CreateAndDeleteUser(objDictionary, driver, strUserName,strPassword);
  		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary, "SentryMobile","True","Parker");
   		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
   		clsCommonMobile.PopulateAction(objDictionary, iosdriver, "Login", "Populate Login", "{T} Email Id|{T} Password",strUserName+"|"+strPassword);
   		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Login", "Login",1);
   		try {Thread.sleep(1000);}catch (Exception e) {}
//   		clsCommonMobile.VerificationPointText(objDictionary,iosdriver, "Account Details", "Do you want to enable biometric authentication for easier login?",1,"Value","Do you want to enable biometric authentication for easier login?");
//  		clsCommonMobile.ClickButton(objDictionary, iosdriver, "Account Details", "Not right now",1);
  		clsCommonMobile.VerificationPointText(objDictionary, iosdriver, "Login", "Dialog Message-Invalid Credentials.", 1, "Exists", "");
  		iosdriver.quit();
   		clsMeter.METER_SetMeterEndTime(objDictionary);
   		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	...
  	@Test(priority=3018)
  	public void I3018_ReimburseParker_ValidateMobileAccountBalance()
  	{
		objDictionary.put("strAssociatedBug", "175470307|146986593");
  		objDictionary.put("strMobileDeviceType", "IOS");
  		CommonIOS clsCommonMobile = new CommonIOS();
  		Meter clsMeter = new Meter();
  		clsCommonMobile.SENTRYMOBILE_AddReportVariables(objDictionary);
  		...
  		clsCommonWeb.SENTRYLINK_SearchUserByOnApprovedUserPage(objDictionary, driver, "parker");
  		String strUniqueId = objDictionary.get("strUniqueId");
		String strMunicipality = objDictionary.get("strMunicipality");
		//String strFirstName = strUniqueId+strMunicipality.replace(" ", "").replace(",","")+" parker";
		String strFirstName = "parker, "+strUniqueId+strMunicipality.replace(" ", "").replace(",","");
		objDictionary.put("strWaitForPageLoad","False");
		try {Thread.sleep(1000);}catch (Exception e) {}
		clsCommonWeb.ClickLink(objDictionary, driver, "Approved Users", strFirstName, 1);
		//Capture Current Account Balance
		clsCommonWeb.GlobalObjectWait(objDictionary, driver, "User", "{TextFieldExists} User~Reimburse Amount", 40);
		objDictionary.put("strWaitForPageLoad","True");
		String strCurrentAccountBalance = clsCommonWeb.StoreText(objDictionary, driver, "User", "Account Balance", 1, "strCurrentAccountBalance");
		//Populate  Reimburse Amount > $20.00
		clsCommonWeb.PopulateAction(objDictionary, driver, "User", "Populate Reimburse Amount", "{T} Reimburse Amount", "101.00");
		clsCommonWeb.ClickButton(objDictionary, driver,  "User", "Reimburse Funds", 1,"Local");
		//Validate the Current Account Balance didn't change
		clsCommonWeb.VerificationPointText(objDictionary, driver, "User", "Account Balance", 1, "Value", strCurrentAccountBalance);
		//Populate Reimburse Amount < $20.00
		clsCommonWeb.PopulateAction(objDictionary, driver, "User", "Populate Reimburse Amount", "{T} Reimburse Amount", "20.00");
		objDictionary.put("strWaitForPageLoad","False");
		clsCommonWeb.ClickButton(objDictionary, driver,  "User", "Reimburse Funds", 1,"Local");
		//Validate Account Balance
		String strReimburseAmount = objDictionary.get("strReimburseAmount").replace("$", "");
		Double dblNewAccountBalance = Double.parseDouble(strCurrentAccountBalance.replace("$", "")) + Double.parseDouble(strReimburseAmount);
		DecimalFormat dFormat = new DecimalFormat("$#.00");//"$#.##" //$#.00
		clsCommonWeb.ClickButton(objDictionary, driver,  "User", "Reimburse Funds", 1,"Local");
		objDictionary.put("strWaitForPageLoad","True");
		clsCommonWeb.VerificationPointText(objDictionary, driver, "User", "Account Balance", 1, "Value", dFormat.format(dblNewAccountBalance));
		driver.quit();
		//Open iOS App
		IOSDriver iosdriver = clsCommonMobile.SetMobileDriver(objDictionary,"SentryMobile","True","Parker");
  		clsCommonMobile.SENTRYMOBILE_Open(objDictionary, iosdriver);
  		clsCommonMobile.SENTRYMOBILE_LoginWithoutCapsRememberOff(objDictionary, iosdriver, "parker", "ff", "Remind me later");
  		//Navigate to Accounts
  		clsCommonMobile.SENTRYMOBILE_NavigateToPageUsingMenuButtons(objDictionary, iosdriver, "Account Details");
  		//clsCommonMobile.GlobalWait(objDictionary, iosdriver, "Account Details", "{TextFieldExists} Account Details~Account Balance", 10);
  		clsCommonMobile.VerificationPointTextField(objDictionary, iosdriver, "Account Details", "Account Balance", 1, "Value", dFormat.format(dblNewAccountBalance));
  		iosdriver.quit();
  		clsMeter.METER_SetMeterEndTime(objDictionary);
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	...
  	@Test(priority=3019)
  	public void I3019_FTFP0_PS1_CMM_MP1_ES1_VPSH_VICAE_VIAC()
  	{
		String strMethondName = new Object(){}.getClass().getEnclosingMethod().getName();
  		...
  		CommonIOS clsCommonMobile = new CommonIOS();
		clsCommonMobile.SENTRYMOBILE_3019_FTFP0_PS1_CMM_MP1_ES1_VPSH_VICAE_VIAC(objDictionary,strLicensePlateNumber);
		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3020)
  	public void I3020_VM_FTFP0_PS1_CMM_MP1_ES1_VPSH_VICAE_VIAC()
  	{
  		CommonIOS clsCommonMobile = new CommonIOS();
		...
		String strMethondName = new Object(){}.getClass().getEnclosingMethod().getName();
  		...
  		clsCommonMobile.SENTRYMOBILE_3020_VM_FTFP0_PS1_CMM_MP1_ES1_VPSH_VICAE_VIAC(objDictionary,strLicensePlateNumber);
  		String strAssociatedBug = objDictionary.get("strAssociatedBug");if(strAssociatedBug == null){strAssociatedBug = "";}
		if(!strAssociatedBug.equals("")){Reporter.log("<font color='Blue'>"+strAssociatedBug+"-This Test passed remove the AssociatedBug</font>");}
  	}
  	@Test(priority=3021)
    ...
    {
  		objDictionary.put("strAssociatedBug", "188731741|177295054|153460868");
		CommonIOS clsCommonMobile = new CommonIOS();
		CommonWeb clsCommonWeb = new CommonWeb();
		GlobalClass clsGlobal = new GlobalClass();
		//Function Variables
